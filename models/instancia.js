...
class Instancia{
  ...
  async maiorTempoSemAcesso() {
    return new Promise(async (resolve, reject) => {
        try {
            const dados = await collection.find({}).sort({ _id: 1 }).toArray();

            if (dados.length < 2) {
                resolve({ maiorTempoSemAcesso: 0 }); // Retorna 0 se não houver registros suficientes
                return;
            }

            let maiorTempoSemAcesso = 0;

            for (let i = 0; i < dados.length - 1; i++) {
                const dataAtualParts = dados[i].data.split(' ')[0].split('/'); // ['18', '10', '2024']
                const dataProximoParts = dados[i + 1].data.split(' ')[0].split('/'); // ['19', '10', '2024']

                const dataAtual = new Date(`${dataAtualParts[2]}-${dataAtualParts[1]}-${dataAtualParts[0]}T00:00:00`); // YYYY-MM-DD
                const dataProximo = new Date(`${dataProximoParts[2]}-${dataProximoParts[1]}-${dataProximoParts[0]}T00:00:00`); // YYYY-MM-DD

                const diferenca = (dataProximo - dataAtual) / (1000 * 60 * 60 * 24); // Diferença em dias
                if (diferenca > maiorTempoSemAcesso) {
                    maiorTempoSemAcesso = diferenca;
                }
            }

            resolve({ maiorTempoSemAcesso }); // Retorna o número de dias em um objeto JSON
        } catch (err) {
            reject(err);
        }
    });
